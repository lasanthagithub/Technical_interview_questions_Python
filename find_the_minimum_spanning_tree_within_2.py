...
def question3(g):
	...
	graph.set_node_names(g_key_list)  # 6 

	## convert g to Graph. Insert edges and nodes
	for node in g_key_list:
		for edge in g[node]:
			graph.insert_edge(int(edge[1]), g_key_list.index(node), g_key_list.index(edge[0]))
	
	## Get potential paths
	paths = []
	for i in range(len(g_key_list)):
		paths.append(graph.bfs_names(i))
